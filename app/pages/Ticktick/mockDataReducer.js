  ...
  produce(state, draft => {
    switch (action.type) {
      case ADD_TASK: {
        ...
        draft.insertableLists[action.payload.selectedList.type][
          action.payload.selectedList.listID
        ].tasks.push(guid);
      ...
      case TOGGLE_DONE:
        draft.tasks[action.payload].completed = !draft.tasks[action.payload]
          .completed;
      ...
      case SORT_LIST:
        ...
        switch (action.payload.sortType) {
          case 'priority':
            draft.insertableLists[action.payload.selectedList.type][
              action.payload.selectedList.listID
            ].tasks = draft.insertableLists[action.payload.selectedList.type][
              action.payload.selectedList.listID
            ].tasks.sort(
              (a, b) => draft.tasks[b].priority - draft.tasks[a].priority,
            );
            break;
          case 'timeAdded':
            draft.insertableLists[action.payload.selectedList.type][
              ...
              (a, b) => draft.tasks[b].timeCreated - draft.tasks[a].timeCreated,
            );
            break;
        }
        break;
      case DELETE_TASK_FROM_LIST:
        draft.insertableLists[action.payload.type][
          action.payload.listID
        ].tasks = without(
          draft.insertableLists[action.payload.type][action.payload.listID]
            .tasks,
      ...
      case ADD_TASK_TO_LIST:
        draft.insertableLists[action.payload.type][
          action.payload.listID
        ].tasks.push(action.payload.taskID);
        break;
    }
  });
