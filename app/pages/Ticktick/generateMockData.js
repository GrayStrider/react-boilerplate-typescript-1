...
export default function([tasks, tags, groups]) {
  for (let i = 0; i < MOCK_TASKS_AMOUNT; i += 1) {
    ...
    tasks[guid] = {
      taskID: guid,
      taskContent: chance.sentence({
        words: chance.integer({ min: 2, max: 6 }),
      }),
  ...
  for (let i = 0; i < 4; i += 1) {
    ...
    groups[guid] = {
      ...
      type: 'groups',
      tasks: randomTasksToDistribute.splice(
        0,
        chance.integer({ min: 20, max: MOCK_TASKS_AMOUNT / 6 }),
      ),
    };
  }
}
